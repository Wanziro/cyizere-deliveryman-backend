...
if (isset($data["email"])) {
	...
	if (validateUser($email,$userId)) {
		$Arr = array();
		$qx = mysqli_query($conn, "select * from product,orders,order_details where order_details.order_id=orders.order_id and order_details.prod_id=product.prod_id and product.supplier_id='$userId' and orders.payment_status='success' order by orders.order_id desc");
		if(mysqli_num_rows($qx) > 0){
			while ($row = mysqli_fetch_assoc($qx)) {
				$obj = new StdClass();
